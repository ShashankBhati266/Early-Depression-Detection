...
def load_phq9(dataset_dir: str) -> Dict[int, float]:
    ...
    The CSV is expected to have columns like:
        uid, score   OR   uid, phq   OR similar
    We auto-detect the score column.
    """
    ...
    uid_col = next((c for c in df.columns if "uid" in c or "user" in c or c == "id"), None)
    if uid_col is None:
        uid_col = df.columns[0]

    # Auto-detect score column
    score_col = next((c for c in df.columns
                      if any(k in c for k in ["score", "phq", "total", "sum", "value"])
                      and c != uid_col), None)
    ...
    print(f"[PHQ-9] uid_col='{uid_col}', score_col='{score_col}'")

    # Some datasets have multiple PHQ-9 rows per user (start/end of term)
    # We take the mean across all measurements
    uid_to_phq = {}
    for _, row in df.iterrows():
        try:
            uid = int(str(row[uid_col]).replace("u", "").replace("U", "").strip())
            score = float(row[score_col])
            if uid not in uid_to_phq:
                uid_to_phq[uid] = []
            uid_to_phq[uid].append(score)
        except (ValueError, KeyError):
            continue

    return {uid: float(np.mean(scores)) for uid, scores in uid_to_phq.items()}
